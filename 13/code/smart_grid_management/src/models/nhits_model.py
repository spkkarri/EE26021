"""
N-HiTS OOF Training — All 3 Tracks
- Paper: Challu et al. 2022 (arxiv 2201.12886)
- MaxPool multi-rate sampling + hierarchical interpolation
- Walk-forward CV with track-specific configs
- Per-fold normalization on train only — no leakage
- Blown fold detection and recovery
...
OUTPUT_DIR = ROOT / "data" / "processed" / "oof" / "nhits"
MODELS_DIR = ROOT / "data" / "processed" / "models" / "nhits"
...
# Track-specific configs — tuned per dataset size
TRACK_CONFIGS = {
    "load": {
        "n_splits"               : 5,
        "hidden_size"            : 512,   # doubled — more capacity
        "n_blocks"               : 3,
        "dropout"                : 0.05,  # less dropout — model was underfitting
        "learning_rate"          : 0.0003,# slightly lower — more stable
        "batch_size"             : 128,   # smaller — better gradient estimates
        "epochs"                 : 150,   # more epochs
        "early_stopping_patience": 15,    # more patience
        "min_delta"              : 0.00005,
        "blown_fold_threshold"   : 5.0,
    },
    "solar": {
        "n_splits"               : 3,
        "hidden_size"            : 256,   # doubled
        "n_blocks"               : 3,
        "dropout"                : 0.1,
        "learning_rate"          : 0.0001,# lower — more stable on small data
        "batch_size"             : 32,    # smaller batches
        "epochs"                 : 200,
        "early_stopping_patience": 20,
        "min_delta"              : 0.00003,
        "blown_fold_threshold"   : 3.0,
    },
    "wind": {
        "n_splits"               : 3,
        "hidden_size"            : 256,   # doubled
        "n_blocks"               : 3,
        "dropout"                : 0.1,
        "learning_rate"          : 0.0001,# lower — wind fold 2 kept blowing
        "batch_size"             : 32,
        "epochs"                 : 200,
        "early_stopping_patience": 20,
        "min_delta"              : 0.00003,
        "blown_fold_threshold"   : 2.0,   # tighter — reject bad folds faster
    },
}
    ...
    def __init__(self, input_size, hidden_size=256, output_size=24,
        ...
        self.blocks = nn.ModuleList([
            NHiTSBlock(
                ...
                pool_kernel=safe_kernels[i],
                expressiveness_ratio=ratios[i],
                dropout=dropout
            )
...
def naive_baseline_oof(X, y, feat_cols):
    if "lag_24" in feat_cols:
        ...
        print("  ⚠ No lag feature found, using zeros")
...
# STEP 4: N-HiTS OOF
# ─────────────────────────────────────────────
def run_nhits_oof(X, y, global_config, track_cfg, track_name, naive_mae):
    device   = global_config["device"]
    horizon  = global_config["horizon"]
    n_splits = track_cfg["n_splits"]
    blown_threshold = naive_mae * track_cfg["blown_fold_threshold"]

    tscv       = TimeSeriesSplit(n_splits=n_splits)
    oof_preds  = np.full((len(X), horizon), np.nan, dtype=np.float32)
    valid_mask = np.zeros(len(X), dtype=bool)

    input_size = int(np.prod(X.shape[1:]))
    print(f"  input_size (flattened): {input_size}")
    print(f"  n_splits: {n_splits} | blown threshold: {blown_threshold:.4f}")

    for k in [24, 4]:
        if input_size % k != 0:
            print(f"  ⚠ input_size={input_size} not divisible by {k} → kernel fallback to 1")
    ...
    for fold, (train_idx, val_idx) in enumerate(tscv.split(X)):
        ...
        # Clip to prevent extreme values causing divergence
        ...
        # Tensors
        X_tr_t = torch.tensor(X_tr_2d, dtype=torch.float32)
        y_tr_t = torch.tensor(y_train, dtype=torch.float32)
        X_vl_t = torch.tensor(X_vl_2d, dtype=torch.float32)
        ...
        model = NHiTSModel(
            input_size=input_size,
            hidden_size=track_cfg["hidden_size"],
            output_size=horizon,
            n_blocks=track_cfg["n_blocks"],
            dropout=track_cfg["dropout"],
        ).to(device)

        # Weight initialization — helps prevent divergence
        ...
        for epoch in range(track_cfg["epochs"]):
            ...
            if avg_val > 1000 or np.isnan(avg_val):
                print(f"    ⚠ Diverged at epoch {epoch+1} (val={avg_val:.2f}) — stopping fold")
                break
        ...
        if best_state is None or best_val_loss > 100:
            print(f"  ⚠ Fold {fold+1} failed to converge — skipping")
            blown_folds += 1
            continue
...
def evaluate(y_true, y_pred, naive_pred, mask, track_name):
    ...
    print(f"\n{'='*58}")
    print(f"  {track_name.upper()} — N-HiTS EVALUATION")
    print(f"{'='*58}")
    print(f"  {'Metric':<12} {'N-HiTS':>10} {'Naive':>10} {'Improve':>10}")
    print(f"  {'-'*50}")
...
def print_statistical_summary(y_true, y_pred, naive_pred, mask, track_name):
    ...
    for label, pred in [("N-HiTS", y_p), ("Naive", y_n)]:
        res = y_t - pred
        print(f"\n  {label} predictions:")
...
def plot_results(y_true, y_pred, naive_pred, mask,
    ...
    horizon_mae_nhits = np.mean(np.abs(y_t - y_p), axis=0)
    horizon_mae_naive = np.mean(np.abs(y_t - y_n), axis=0)
    horizon_r2 = []
    for h in range(24):
        ss_r = np.sum((y_t[:, h] - y_p[:, h]) ** 2)
        ss_t = np.sum((y_t[:, h] - y_t[:, h].mean()) ** 2)
        horizon_r2.append(1 - ss_r / ss_t)

    window_mae_nhits = np.mean(np.abs(y_t - y_p), axis=1)
    window_mae_naive = np.mean(np.abs(y_t - y_n), axis=1)
    sorted_idx = np.argsort(window_mae_nhits)
    ...
    fig.suptitle(
        f"N-HiTS vs Naive Baseline — {track_name.upper()} Track\n"
        f"N-HiTS: MAE={metrics['nhits_mae']:.4f} | "
        f"RMSE={metrics['nhits_rmse']:.4f} | "
        f"R²={metrics['nhits_r2']:.4f} | "
        f"r={metrics['nhits_corr']:.4f}\n"
    ...
    ax1.plot(hours, horizon_mae_nhits, color="#2196F3", linewidth=2,
             marker="o", markersize=3, label="N-HiTS")
    ax1.plot(hours, horizon_mae_naive, color="#e74c3c", linewidth=2,
             linestyle="--", marker="x", markersize=3, label="Naive")
    ax1.fill_between(hours, horizon_mae_nhits, horizon_mae_naive,
                     alpha=0.15, color="#2196F3", label="N-HiTS advantage")
    ...
    r2_clipped = np.clip(horizon_r2, 0, 1)
    ax2.bar(hours, r2_clipped, color="#9b59b6", alpha=0.85)
    ax2.axhline(0.85, color="red", linestyle="--",
                linewidth=1.5, label="Target R²=0.85")
    mean_r2 = np.mean([r for r in horizon_r2 if r > 0])
    ...
    ax3.hist(window_mae_nhits, bins=50, color="#2196F3",
             alpha=0.6, label="N-HiTS")
    ax3.hist(window_mae_naive, bins=50, color="#e74c3c",
             alpha=0.6, label="Naive")
    ax3.axvline(window_mae_nhits.mean(), color="#2196F3", linewidth=2,
                linestyle="--",
                label=f"N-HiTS={window_mae_nhits.mean():.3f}")
    ...
    ax4.scatter(y_t[sample].flatten(), y_p[sample].flatten(),
                alpha=0.2, s=1, color="#2196F3", label="N-HiTS")
    ax4.scatter(y_t[sample].flatten(), y_n[sample].flatten(),
                alpha=0.15, s=1, color="#e74c3c", label="Naive")
    lims = [
        min(y_t.min(), y_p[y_p > -5].min() if (y_p > -5).any() else -1),
        max(y_t.max(), y_p[y_p < 5].max()  if (y_p < 5).any()  else 2)
    ]
    ...
    for ax, idx, label in [
        ...
        mae_nhits = np.mean(np.abs(y_t[idx] - y_p[idx]))
    ...
    res_nhits = np.clip((y_t - y_p).flatten(), -3, 3)
    res_naive = (y_t - y_n).flatten()
    ax8.hist(res_nhits, bins=60, color="#2196F3",
             alpha=0.6, label=f"N-HiTS (bias={res_nhits.mean():.4f})")
    ...
    sorted_nhits = np.sort(window_mae_nhits)
    sorted_naive = np.sort(window_mae_naive)
    cum = np.arange(1, len(sorted_nhits) + 1) / len(sorted_nhits)
    ax9.plot(sorted_nhits, cum, color="#2196F3", linewidth=2,
             label=f"N-HiTS (mean={window_mae_nhits.mean():.3f})")
    ...
    save_path = output_dir / f"nhits_{track_name}_oof_results.png"
...
def main():
    ...
    for track_name, fname in TRACKS.items():
        ...
        print(f"\n  Running N-HiTS OOF on {CONFIG['device'].upper()} "
              f"[n_splits={track_cfg['n_splits']} | "
              f"hidden={track_cfg['hidden_size']} | "
              f"lr={track_cfg['learning_rate']}]...")

        oof_preds, valid_mask = run_nhits_oof(
            X, y, CONFIG, track_cfg, track_name, naive_mae
        )

        # Only evaluate on valid (non-blown) windows
        if valid_mask.sum() == 0:
            print(f"  ❌ No valid predictions for {track_name} — skipping")
            continue
        ...
        np.save(track_out / "nhits_oof.npy",  oof_preds)
        np.save(track_out / "nhits_mask.npy", valid_mask)
    ...
    if all_metrics:
        print(f"\n{'='*68}")
        print(f"FINAL SUMMARY — N-HiTS vs Naive")
        print(f"{'='*68}")
        print(f"{'Track':<8} {'MAE':>8} {'Naive':>8} "
              f"{'R²':>8} {'sMAPE':>8} {'Improve':>10}")
        print("-" * 68)
        for m in all_metrics:
            print(f"{m['track']:<8} {m['nhits_mae']:>8.4f} "
                  f"{m['naive_mae']:>8.4f} {m['nhits_r2']:>8.4f} "
                  f"{m['nhits_smape']:>7.1f}% {m['improve']:>9.1f}%")
